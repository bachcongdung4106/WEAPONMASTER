...
bool MainObject::LoadImg(std::string path, SDL_Renderer* screen)
{
	...
	if (ret == true) 
	{
		width_frame_ = rect_.w / 6;//kich co 1 o,thay bang so sprite
...
{
	if (width_frame_ > 0 && height_frame_ > 0)// co the dung vong lap
	{
		frame_clip_[0].x = 0;
		frame_clip_[0].y = 0;
...
void MainObject::Show(SDL_Renderer* des, const Map& map_data)
{
	...
	if (status_ != prev_status_)
	{
		...
		{
			LoadImg("img/spritesheet/walkleft.png", des);
		}
		else
		{
			LoadImg("img/spritesheet/walkright.png", des);
		}
...
{
	map_data.start_x_ = x_pos_ - (SCREEN_WIDTH / 2);//khi den 1/2 map thi moi di chuyen
